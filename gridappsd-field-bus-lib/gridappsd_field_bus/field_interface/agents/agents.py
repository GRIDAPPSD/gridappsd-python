...
def compute_req(cls):
        ...
        def wrapper(*args, **kwargs):
            ...
            if func.__name__ == "__init__":
                decorator_logger.info(
                    f"{class_name}.{func.__name__}.{args[0].agent_id} took: {end - start:.6f} seconds"
                )
            return result
    ...
    def get_deep_size(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            decorator_logger.info(
                f"{self.__class__.__name__}.{func.__name__}.{args[0].agent_id} size is: {obj_size} bytes"
            )
        ...
        def wrapper(*args, **kwargs):
            ...
            for name in area_names:
                ...
                if area_dict is not None and hasattr(area_dict, "graph"):
                    ...
                    decorator_logger.info(
                        f"{self.__class__.__name__}.{func.__name__}.{args[0].agent_id} length of graph: {size}"
                    )
                    decorator_logger.info(
                        f"{self.__class__.__name__}.{name}.{args[0].agent_id} graph keys: {graph_keys}"
                    )
                    area_found = True
                    break
            ...
            if not area_found:
                decorator_logger.error(
                    f"{class_name}.{func.__name__}.{args[0].agent_id} No area dictionary (feeder/switch/secondary) found in {self.__class__.__name__}"
                )
            return result
...
class DistributedAgent:
    ...
    def subscribe_to_messages(self):
        ...
        self.downstream_message_bus.subscribe(
            t.field_message_bus_app_topic(self.downstream_message_bus.id, self.app_id), self.on_downstream_message
        )
        self.upstream_message_bus.subscribe(
            t.field_message_bus_app_topic(self.upstream_message_bus.id, self.app_id), self.on_upstream_message
        )
        ...
        if "context_manager" not in self.app_id:
            ...
            self.upstream_message_bus.subscribe(
                t.field_message_bus_agent_topic(self.upstream_message_bus.id, self.agent_id), self.on_upstream_message
            )
    ...
    def subscribe_to_requests(self):
        ...
        self.downstream_message_bus.subscribe(
            t.field_agent_request_queue(self.downstream_message_bus.id, self.agent_id), self.on_request_from_downstream
        )
        self.upstream_message_bus.subscribe(
    ...
    def get_registration_details(self):
        details = AgentRegistrationDetails(
            str(self.agent_id),
            self.app_id,
            self.description,
...
class SubstationAgent(DistributedAgent):
    def __init__(
        ...
        super().__init__(
            upstream_message_bus_def, downstream_message_bus_def, agent_config, substation_dict, simulation_id
        )
...
class SwitchAreaAgent(DistributedAgent):
    def __init__(
        ...
        super().__init__(
            upstream_message_bus_def, downstream_message_bus_def, agent_config, switch_area_dict, simulation_id
        )
...
class SecondaryAreaAgent(DistributedAgent):
    def __init__(
        ...
        super().__init__(
            upstream_message_bus_def, downstream_message_bus_def, agent_config, secondary_area_dict, simulation_id
        )
