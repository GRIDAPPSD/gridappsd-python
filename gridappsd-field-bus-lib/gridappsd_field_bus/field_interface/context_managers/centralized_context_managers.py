...
from gridappsd_field_bus.field_interface.context_managers.context_manager_agents import (
...
def _main():
    ...
    feeder_agent = FeederAreaContextManager(
        system_message_bus_def, feeder_message_bus_def, agent_config, simulation_id=simulation_id
    )
    ...
    for switch_area in feeder_agent.agent_area_dict["SwitchAreas"]:
        ...
        for secondary_area in switch_area["SecondaryAreas"]:
            secondary_area_message_bus_def = get_message_bus_definition(str(secondary_area["@id"]))
